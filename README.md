...
### Analytics tools (unique to this server)
...
```json
{
  "mcpServers": {
    "trading212": {
      "command": "uv",
      "args": ["run", "--directory", "<path-to-repo>", "src/server.py"],
      "env": {
...
```bash
claude mcp add trading212 -- uv run --directory <path-to-repo> src/server.py
```
